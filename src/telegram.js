...
getName(userId, form = {}) {
  ...
  /**
 * Use this method to generate or retrieve a primary invite link for a chat.
 * 
 * The bot must be an administrator in the chat and must have the appropriate admin rights.
 * Every administrator in a chat generates their own invite links. Bots can't use invite links
 * generated by other administrators. If you want your bot to work with invite links, it will need
 * to generate its own link using this method.
 *
 * @param  {Number|String} chatId   Unique identifier for the target chat or username of the target channel (in the format `@channelusername`)
 * @param  {Object} [options] Additional Telegram query options
 * @return {Promise} Promise resolving to a string containing the invite link
 * @see https://core.telegram.org/bots/api#exportchatinvitelink
 */
getChatInviteLink(chatId, options = {}) {
  const form = {
    chat_id: chatId,
    ...options
  };
  ...
  return this._request('exportChatInviteLink', { form })
    .then(result => {
      // The API returns the invite link directly as a string
      return result;
    })
    .catch(error => {
      console.error('Error generating chat invite link:', error);
